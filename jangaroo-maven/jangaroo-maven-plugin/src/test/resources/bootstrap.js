var Ext = Ext || {};
Ext.manifest = Ext.manifest || "testModuleManifest.json";
// @tag core
// @define Ext.Boot
...
Ext.Boot = Ext.Boot || (function (emptyFn) {
                  ...
                  Boot = {
                    ...
                    init: function () {
                      ...
                      if (!baseUrl) {
                        ...
                        baseUrl = script ? script.src : "file://" + Envjs.getcwd() + "/src/test/resources/";
                        Boot.hasReadyState = script && ('readyState' in script);
                        Boot.hasAsync = !script || ("async" in script) || !Boot.hasReadyState;
                        if (!script) {
                          Ext.manifest = baseUrl + Ext.manifest;
                        }
                      }
                      ...
                      Boot.origin = origin.indexOf('undefined') > -1 ? baseUrl : origin;
                    ...
                    canonicalUrl: function (url) {
                      _debug("computing canonical URL for " + url);
                    ...
                    getEntry: function (url, cfg, canonicalPath) {
                      ...
                      _debug("entry for url " + url + " is " + entry);
                    ...
                    processRequest: function(request, sync) {
                      _debug("processing request " + request + " with sync " + sync);
                      ...
                      var xhr = new XMLHttpRequest(),
                              ...
                              readyStateChange = function () {
                                if (xhr && xhr.readyState == 4) {
                                  ...
                                  if (complete) {
                                    _debug("calling " + complete.toSource().trunc(32, false) + " with argument " + result);
          ...
          Request.prototype = {
            ...
            getEntries: function () {
              ...
              _debug("getting entries " + entries);
            ...
            loadEntries: function(sync) {
              ...
              _debug("loading " + len + " entries " + entries + " with sync " + me.sync + ", starting at " + start);
            ...
            processLoadedEntries: function () {
              ...
              _debug("processing " + len + " loaded entries");
              ...
              if (!me.done) {
                for (i = start; i < len; i++) {
                  ...
                  if (entry.error) {
                    console.error("entry " + entry + " has error");
                    me.error = true;
                  }
                }
                me.notify();
          ...
          Entry.prototype = {
            ...
            createLoadElement: function(callback) {
              ...
              if(Boot.hasReadyState) {
                _debug("attaching onreadystatechange to load element for " + me.url);
                el.onreadystatechange = readyStateChange;
              } else {
                _debug("attaching onload to load element for " + me.url);
                el.onload = callback;
              }
            ...
            loadCrossDomain: function() {
              ...
              _debug("loading cross domain entry " + me);
            ...
            loadElement: function() {
              ...
              _debug("loading element for entry " + me);
              me.createLoadElement(function(){
                _debug("loaded element for entry " + me);
                complete();
              });
              me.evaluateLoadElement();
              return true;
            ...
            loadSync: function() {
              var me = this;
              _debug("loading entry " + me + " synchronously");
              me.fetch({
                async: false,
              ...
              if (!me.loaded) {
                ...
                if (!sync) {
                  ...
                  // document
                    ...
                    _debug("loading entry " + me + " (option 1)");
                    me.createLoadElement(function () {
                      _debug("loaded entry " + me + ", notifying request");
                      me.loaded = true;
                      me.notifyRequests();
                    });
                  }
                ...
                _debug("already loaded entry " + me);
              }
            ...
            evaluateLoadElement: function() {
              ...
              _debug("evaluateLoadElement called for " + element);
              Boot.getHead().appendChild(element);
            },

            evaluate: function () {
              var me = this;
              if(!me.evaluated) {
                _debug("evaluating " + me);
                if(me.evaluating) {
                  return;
                }
                me.evaluating = true;
