...
/**
 * The String class is a data type that represents a string of characters. The String class provides methods and
 * properties that let you manipulate primitive string value types. You can convert the value of any object into a
 * String data type object using the <code>String()</code> function.
 *
 * <p>Because all string indexes are zero-based, the index of the last character for any string x is x.length - 1.
 *
 * <p>You can call any of the methods of the String class whether you use the constructor method
 * <code>new String()</code> to create a new string variable or simply assign a string literal value.
 * Unlike previous versions of ActionScript, it makes no difference whether you use the constructor, the global
 * function, or simply assign a string literal value. The following lines of code are equivalent:
 * <pre>
...
public class String {
   ...
   * <p><b>Note:</b> Because string literals use less overhead than String objects and are generally easier to use,
   * you should use string literals instead of the String class unless you have a good reason to use a String object
   ...
   * If index is not a number from 0 to string.length - 1, an empty string is returned.
   *
   * <p>This method is similar to <code>String.charCodeAt()</code> except that the returned value is a character,
   * not a 16-bit integer character code.
   *
   * @param index An integer specifying the position of a character in the string.
   *   The first character is indicated by 0, and the last character is indicated by my_str.length - 1.
   * @return String The character at the specified index. Or an empty string if the specified index is outside the
   *   range of this string's indices.
   *
   * @see #charCodeAt()
   ...
   * If index is not a number from 0 to  string.length - 1, NaN is returned.
   *
   * <p>This method is similar to <code>String.charAt()</code> except that the returned value is a 16-bit integer
   * character code, not the actual character.
   *
   * @param index An integer that specifies the position of a character in the string.
   *   The first character is indicated by 0, and the last character is indicated by my_str.length - 1.
   * @return Number The Unicode character code of the character at the specified index.
  ...
  /**
   * Appends the supplied arguments to the end of the String object, converting them to strings if necessary, and
   * returns the resulting string. The original value of the source String object remains unchanged.
   *
   * @param args Zero or more values to be concatenated.
   * @return String A new string consisting of this string concatenated with the specified parameters.
   *
   ...
   *   Unicode values are defined in the <a href="http://www.unicode.org/ucd/">Unicode Character Database specification</a>.
   ...
   * @return int The index of the first occurrence of the specified substring or <code>-1</code>.
   * @see #lastIndexOf
   ...
   */
  public native function indexOf(val:String, startIndex:Number = null):int;
   ...
   * meaning that the first character is at index 0, and the last is at <code>string.length - 1</code>.
   * If <code>val</code> is not found, the method returns <code>-1</code>.
   *
   ...
   *
   * <ul>
   * <li>If the <code>g</code> (global) flag is <i>not</i> set,
   * the return array will contain no more than one match, and the <code>lastIndex</code>
   * property of the regular expression remains unchanged.</li><li>If the <code>g</code> (global) flag <i>is</i> set, the method starts the search at
   * the beginning of the string (index position 0). If a matching substring is an empty string (which
   * can occur with a regular expression such as <code>/x&lowast;/</code>), the method adds that
   ...
   *
   * <p>If no match is found, the method returns an empty array. If you pass
  ...
  /**
   * Matches the specified <code>pattern</code> against the string and returns a new string
   ...
   *
   * </p><p>In the following example, only the first instance of "sh" (case-sensitive)
   * is replaced: </p>
   * <pre>
   * var myPattern:RegExp = /sh/;
   * var str:String = "She sells seashells by the seashore.";
   * trace(str.replace(myPattern, "sch"));
   *    // She sells seaschells by the seashore.</pre>
   *
   ...
   * var str:String = "She sells seashells by the seashore.";
   * trace(str.replace(myPattern, "sch"));
   *    // She sells seaschells by the seaschore.</pre>
   *
   ...
   * and the matches are <i>not</i> case-sensitive because the <code>i</code> (ignoreCase) flag is set:</p>
   * <pre>
   * var myPattern:RegExp = /sh/gi;
   * var str:String = "She sells seashells by the seashore.";
   * trace(str.replace(myPattern, "sch"));
   *    // sche sells seaschells by the seaschore.</pre>
   *
   * @param pattern The pattern to match, which can be any type of object, but it is typically
   ...
   * as the <code>pattern</code> parameter, you can use the following special <i>$ replacement codes</i>
   ...
   * <tr><td><code>$`</code></td><td>The portion of the string that precedes the matched substring.
   ...
   *   character (‘).
   *   </td></tr>
   * <tr><td><code>$'</code></td><td>The portion of string that follows the matched substring.
   *   Note that this code uses the straight single quote character (').
   *   </td></tr><tr><td><code>$</code><i>n</i>
   *   </td><td>The <i>n</i>th captured parenthetical group match, where <i>n</i> is a single
   *   digit 1-9 and <code>$</code><i>n</i> is not followed by a decimal digit.
   *   </td></tr>
   * <tr><td><code>$</code><i>nn</i></td><td>The <i>nn</i>th captured parenthetical group match, where <i>nn</i> is a two-digit
   ...
   * trace(str.replace(pattern, "$2-$1")); // flop-flip</pre>
   *
   ...
   * <li>Any captured parenthetical group matches are provided as the next arguments. The number of arguments passed
   ...
   * of arguments is passed to the function:</p>
   * <pre>
   * {"abc12", "abc", "12", 0, "abc12 def34"}</pre>
   *
   * <p>The second time, the pattern matches the substring <code>"def23"</code>, and the
   * following list of arguments is passed to the function:</p>
   * <pre>
   * {"def34", "def", "34", 6, "abc123 def34"}</pre>
   *
   * @param options in JavaScript, RegExp options can be given as a separate options string.
  ...
  /**
   * Searches for the specified <code>pattern</code> and returns the index of
   ...
   * @return int The index of the first matching substring, or <code>-1</code> if
  ...
  /**
   * Returns a string that includes the <code>startIndex</code> character and all characters up to, but not including,
   * the <code>endIndex</code> character. The original String object is not modified.
   * If the <code>endIndex</code> parameter is not specified, then the end of the substring is the end of the string.
   * If the character indexed by <code>startIndex</code> is the same as or to the right of the character indexed by
   ...
   *
   * @return String A copy of this string with all uppercase characters converted to lowercase.
   *
   * @see #toLowerCase
   * @return
   */
   ...
   * </pre>
   *
   * <p>These case mappings are defined in the <a href="http://www.unicode.org/ucd/" target="external">Unicode Character Database</a> specification. </p>
   *
   * @return String A copy of this string with all uppercase characters converted to lowercase.
   *
   * @see #toUpperCase
   ...
   * <p>These case mappings are defined in the <a href="http://www.unicode.org/ucd/" target="external">Unicode Character Database</a> specification. </p>
   *
   * @return String A copy of this string with all lowercase characters converted to uppercase.
   *
   * @see #toLowerCase
   * @see http://help.adobe.com/en_US/as3/dev/WS5b3ccc516d4fbf351e63e3d118a9b90204-7eff.html Converting strings between uppercase and lowercase
   */
  ...
  /**
   * Returns the primitive value of a String instance. This method is designed to convert a String object into a
   * primitive string value. Because Flash runtimes automatically call valueOf() when necessary, you rarely need to call
