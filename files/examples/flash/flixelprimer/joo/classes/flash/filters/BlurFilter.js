 ...
 * <p>This filter supports Stage scaling. However, it does not support general scaling, rotation, and skewing. If the object itself is scaled (<code>scaleX</code> and <code>scaleY</code> are not set to 100%), the filter effect is not scaled. It is scaled only when the user zooms in on the Stage.</p>
 ...
 * <p><a href="http://help.adobe.com/en_US/FlashPlatform/reference/actionscript/3/flash/filters/BlurFilter.html#includeExamplesSummary">View the examples</a></p>
...
"public final class BlurFilter extends flash.filters.BitmapFilter",2,function($$private){;return[ 
  ...
  /**
   * The number of times to perform the blur. The default value is <code>BitmapFilterQuality.LOW</code>, which is equivalent to applying the filter once. The value <code>BitmapFilterQuality.MEDIUM</code> applies the filter twice; the value <code>BitmapFilterQuality.HIGH</code> applies it three times and approximates a Gaussian blur. Filters with lower values are rendered more quickly.
   * <p>For most applications, a <code>quality</code> value of low, medium, or high is sufficient. Although you can use additional numeric values up to 15 to increase the number of times the blur is applied, higher values are rendered more slowly. Instead of increasing the value of <code>quality</code>, you can often get a similar effect, and with faster rendering, by simply increasing the values of the <code>blurX</code> and <code>blurY</code> properties.</p>
   * <p>You can use the following BitmapFilterQuality constants to specify values of the <code>quality</code> property:</p>
   * <ul>
   ...
   * @param blurX The amount to blur horizontally. Valid values are from 0 to 255.0 (floating-point value).
   * @param blurY The amount to blur vertically. Valid values are from 0 to 255.0 (floating-point value).
   * @param quality The number of times to apply the filter. You can specify the quality using the BitmapFilterQuality constants:
   * <ul>
   * <li><code>flash.filters.BitmapFilterQuality.LOW</code></li>
   * <li><code>flash.filters.BitmapFilterQuality.MEDIUM</code></li>
   * <li><code>flash.filters.BitmapFilterQuality.HIGH</code></li></ul>
   * <p>High quality approximates a Gaussian blur. For most applications, these three values are sufficient. Although you can use additional numeric values up to 15 to achieve different effects, be aware that higher values are rendered more slowly.</p>
   *
   */
  "public function BlurFilter",function BlurFilter$(blurX/*:Number = 4.0*/, blurY/*:Number = 4.0*/, quality/*:int = 1*/) {if(arguments.length<3){if(arguments.length<2){if(arguments.length<1){blurX = 4.0;}blurY = 4.0;}quality = 1;}this.super$2();
    throw new Error('not implemented'); // TODO: implement!
  },

  /**
   * Returns a copy of this filter object.
   * @return A new BlurFilter instance with all the same properties as the original BlurFilter instance.
   *
   */
