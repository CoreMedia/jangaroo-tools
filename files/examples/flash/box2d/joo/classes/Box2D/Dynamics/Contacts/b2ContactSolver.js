...
"public class b2ContactSolver",1,function($$private){var assert=joo.assert;return[function(){joo.classLoader.init(Box2D.Common.b2Settings);},
	...
	"public function b2ContactSolver",function b2ContactSolver$()
	{this.m_step$1=this.m_step$1();this.m_constraints=this.m_constraints();
	},
	
	"private static var",{ s_worldManifold/*:b2WorldManifold*/ :function(){return( new Box2D.Collision.b2WorldManifold());}},
	"public function Initialize",function Initialize(step/*:b2TimeStep*/, contacts/*:Array*//*b2Contact*/, contactCount/*:int*/, allocator/*:**/)/*:void*/
	{
		...
		for (i = 0; i < contactCount; ++i)
		{
			...
			var friction/*:Number*/ = Box2D.Common.b2Settings.b2MixFriction(fixtureA.GetFriction(), fixtureB.GetFriction());
			var restitution/*:Number*/ = Box2D.Common.b2Settings.b2MixRestitution(fixtureA.GetRestitution(), fixtureB.GetRestitution());
			...
			var normalX/*:Number*/ = $$private.s_worldManifold.m_normal.x;
			var normalY/*:Number*/ = $$private.s_worldManifold.m_normal.y;
			...
			for (var k/*:uint*/ = 0; k < cc.pointCount; ++k)
			{
				...
				var rAX/*:Number*/ = ccp.rA.x = $$private.s_worldManifold.m_points[k].x - bodyA.m_sweep.c.x;
				var rAY/*:Number*/ = ccp.rA.y = $$private.s_worldManifold.m_points[k].y - bodyA.m_sweep.c.y;
				var rBX/*:Number*/ = ccp.rB.x = $$private.s_worldManifold.m_points[k].x - bodyB.m_sweep.c.x;
				var rBY/*:Number*/ = ccp.rB.y = $$private.s_worldManifold.m_points[k].y - bodyB.m_sweep.c.y;
	...
	"public function InitVelocityConstraints",function InitVelocityConstraints(step/*: b2TimeStep*/)/* : void*/{
		var tVec/*:b2Vec2*/;
		var tVec2/*:b2Vec2*/;
		var tMat/*:b2Mat22*/;
		
		// Warm start.
		for (var i/*:int*/ = 0; i < this.m_constraintCount$1; ++i)
		{
	...
	"public function SolveVelocityConstraints",function SolveVelocityConstraints()/* : void*/{
		var j/*:int*/;
	...
	"public function FinalizeVelocityConstraints",function FinalizeVelocityConstraints()/* : void*/
	{
		for (var i/*:int*/ = 0; i < this.m_constraintCount$1; ++i)
		{
			var c/*:b2ContactConstraint*/ = this.m_constraints[ i ];
...
//#else
	// Sequential solver.
	"private static var",{ s_psm/*:b2PositionSolverManifold*/ :function(){return( new Box2D.Dynamics.Contacts.b2PositionSolverManifold());}},
	"public function SolvePositionConstraints",function SolvePositionConstraints(baumgarte/*:Number*/)/*:Boolean*/
	{
		var minSeparation/*:Number*/ = 0.0;
		...
		for (var i/*:int*/ = 0; i < this.m_constraintCount$1; i++)
		{
			...
			var normal/*:b2Vec2*/ = $$private.s_psm.m_normal;
			...
			for (var j/*:int*/ = 0; j < c.pointCount; j++)
			{
				...
				var point/*:b2Vec2*/ = $$private.s_psm.m_points[j];
				var separation/*:Number*/ = $$private.s_psm.m_separations[j];
				...
				// Prevent large corrections and allow slop.
				var C/*:Number*/ = Box2D.Common.Math.b2Math.Clamp(baumgarte * (separation + Box2D.Common.b2Settings.b2_linearSlop), -Box2D.Common.b2Settings.b2_maxLinearCorrection, 0.0);
...
//#endif
	"private var",{ m_step/*:b2TimeStep*/ :function(){return( new Box2D.Dynamics.b2TimeStep());}},
	"private var",{ m_allocator/*:**/:undefined},
	"b2internal var",{ m_constraints/*:Array*//*b2ContactConstraint*/ :function(){return( new Array/*b2ContactConstraint*/ ());}},
