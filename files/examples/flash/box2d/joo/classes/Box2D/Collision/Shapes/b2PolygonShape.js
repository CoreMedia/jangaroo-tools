...
"public class b2PolygonShape extends Box2D.Collision.Shapes.b2Shape",2,function($$private){var is=joo.is,as=joo.as,assert=joo.assert;return[function(){joo.classLoader.init(Number,Box2D.Common.b2Settings);},
	...
	"public override function Copy",function Copy()/*:b2Shape*/ 
	{
		var s/*:b2PolygonShape*/ = new Box2D.Collision.Shapes.b2PolygonShape();
		s.Set(this);
		return s;
	},
	
	"public override function Set",function Set(other/*:b2Shape*/)/*:void*/ 
	{
		this.Set$2(other);
		if (is(other,  Box2D.Collision.Shapes.b2PolygonShape))
		{
			var other2/*:b2PolygonShape*/ =as( other,  Box2D.Collision.Shapes.b2PolygonShape);
	...
	"override public function ReflectX",function ReflectX()/*:void*/
	{
		...
		var t/*:b2Vec2*/;
		var i1/*:int*/ = 0;
		var i2/*:int*/ = this.m_vertexCount-1;
		for (; i1 < i2; ++i1, --i2)
		{
	 ...
	 */
	"public function SetAsArray",function SetAsArray(vertices/*:Array*/, vertexCount/*:Number = 0*/)/*:void*/
	{if(arguments.length<2){vertexCount = 0;}
		var v/*:Array*//*b2Vec2*/ = new Array/*b2Vec2*/();
		for/* each*/(var $1 in vertices)
		{var tVec/*:b2Vec2*/ = vertices[$1];
			v.push(tVec);
		}
		this.SetAsVector(v, vertexCount);
	},
	
	"public static function AsArray",function AsArray(vertices/*:Array*/, vertexCount/*:Number*/)/*:b2PolygonShape*/
	{
		var polygonShape/*:b2PolygonShape*/ = new Box2D.Collision.Shapes.b2PolygonShape();
	 ...
	 * It is assumed that the exterior is the the right of each edge.
	 */
	"public function SetAsVector",function SetAsVector(vertices/*:Array*//*b2Vec2*/, vertexCount/*:Number = 0*/)/*:void*/
	{if(arguments.length<2){vertexCount = 0;}
		if (vertexCount == 0)
		...
		{
			var i1/*:int*/ = i;
			var i2/*:int*/ = i + 1 < this.m_vertexCount ? i + 1 : 0;
			var edge/*:b2Vec2*/ = Box2D.Common.Math.b2Math.SubtractVV(this.m_vertices[i2], this.m_vertices[i1]);/*
			assert edge.LengthSquared() > Number.MIN_VALUE;*/
...
//#endif
	...
	"public static function AsVector",function AsVector(vertices/*:Array*//*b2Vec2*/, vertexCount/*:Number = 0*/)/*:b2PolygonShape*/
	{if(arguments.length<2){vertexCount = 0;}
		var polygonShape/*:b2PolygonShape*/ = new Box2D.Collision.Shapes.b2PolygonShape();
	...
	*/
	"public function SetAsBox",function SetAsBox(hx/*:Number*/, hy/*:Number*/)/* : void*/ 
	{
	...
	"public static function AsBox",function AsBox(hx/*:Number*/, hy/*:Number*/)/*:b2PolygonShape*/
	{
		var polygonShape/*:b2PolygonShape*/ = new Box2D.Collision.Shapes.b2PolygonShape();
	...
	*/
	"static private var",{ s_mat/*:b2Mat22*/ :function(){return( new Box2D.Common.Math.b2Mat22());}},
	"public function SetAsOrientedBox",function SetAsOrientedBox(hx/*:Number*/, hy/*:Number*/, center/*:b2Vec2 = null*/, angle/*:Number = 0.0*/)/* : void*/
	{if(arguments.length<4){if(arguments.length<3){center = null;}angle = 0.0;}
		...
		var xf/*:b2Transform*/ = new Box2D.Common.Math.b2Transform();
		xf.position = center;
		xf.R.Set(angle);
	...
	"public static function AsOrientedBox",function AsOrientedBox(hx/*:Number*/, hy/*:Number*/, center/*:b2Vec2 = null*/, angle/*:Number = 0.0*/)/*:b2PolygonShape*/
	{if(arguments.length<4){if(arguments.length<3){center = null;}angle = 0.0;}
		var polygonShape/*:b2PolygonShape*/ = new Box2D.Collision.Shapes.b2PolygonShape();
	 ...
	 */
	"public function SetAsEdge",function SetAsEdge(v1/*:b2Vec2*/, v2/*:b2Vec2*/)/*:void*/
	{
	 ...
	 * Set this as a single edge.
	 */
	"static public function AsEdge",function AsEdge(v1/*:b2Vec2*/, v2/*:b2Vec2*/)/*:b2PolygonShape*/
	{
		var polygonShape/*:b2PolygonShape*/ = new Box2D.Collision.Shapes.b2PolygonShape();
	...
	* @inheritDoc
	*/
	"public override function TestPoint",function TestPoint(xf/*:b2Transform*/, p/*:b2Vec2*/)/* : Boolean*/{
		var tVec/*:b2Vec2*/;
	...
	"public override function RayCast",function RayCast(output/*:b2RayCastOutput*/, input/*:b2RayCastInput*/, transform/*:b2Transform*/)/*:Boolean*/
	{
		...
		{
			var v1/*:b2Vec2*/ = this.m_vertices[0];
			var v2/*:b2Vec2*/ = this.m_vertices[1];
			var normal/*:b2Vec2*/ = this.m_normals[0];
	 ...
	 * @inheritDoc
	 */
	"public override function ComputeAABB",function ComputeAABB(aabb/*:b2AABB*/, xf/*:b2Transform*/)/* : void*/
	{
	...
	*/
	"public override function ComputeMass",function ComputeMass(massData/*:b2MassData*/, density/*:Number*/)/* : void*/{
		...
		for (var i/*:int*/ = 0; i < this.m_vertexCount; ++i)
		{
			...
			var p3/*:b2Vec2*/ = i + 1 < this.m_vertexCount ? this.m_vertices[$$int(i+1)] : this.m_vertices[0];
	...
	*/
			...
			c/*:b2Vec2*/)/*:Number*/
	{
		...
		var normalL/*:b2Vec2*/ = Box2D.Common.Math.b2Math.MulTMV(xf.R, normal);
		var offsetL/*:Number*/ = offset - Box2D.Common.Math.b2Math.Dot(normal, xf.position);
		...
		var intoVec/*:b2Vec2*/ = new Box2D.Common.Math.b2Vec2(this.m_vertices[intoIndex].x * (1 - intoLamdda) + this.m_vertices[intoIndex2].x * intoLamdda,
										this.m_vertices[intoIndex].y * (1 - intoLamdda) + this.m_vertices[intoIndex2].y * intoLamdda);
		var outoVec/*:b2Vec2*/ = new Box2D.Common.Math.b2Vec2(this.m_vertices[outoIndex].x * (1 - outoLamdda) + this.m_vertices[outoIndex2].x * outoLamdda,
										this.m_vertices[outoIndex].y * (1 - outoLamdda) + this.m_vertices[outoIndex2].y * outoLamdda);
										
		// Initialize accumulator
		var area/*:Number*/ = 0;
		var center/*:b2Vec2*/ = new Box2D.Common.Math.b2Vec2();
	...
	*/
	"public function GetVertices",function GetVertices()/* : Array*//*b2Vec2*/{
	...
	*/
	"public function GetNormals",function GetNormals()/* : Array*//*b2Vec2*/
	{
	 ...
	 */
	"public function GetSupport",function GetSupport(d/*:b2Vec2*/)/*:int*/
	{
		var bestIndex/*:int*/ = 0;
	...
	// TODO: Expose this
	"private function Validate",function Validate()/*:Boolean*/
	{
		/*
	 ...
	 * @private
	 */
	"public function b2PolygonShape",function b2PolygonShape$(){this.super$2();
	...
	"private function Reserve",function Reserve(count/*:int*/)/*:void*/
	{
	...
	"b2internal var",{ m_centroid/*:b2Vec2*/:null},

	"b2internal var",{ m_vertices/*:Array*/:null}/*b2Vec2*/,
	"b2internal var",{ m_normals/*:Array*/:null}/*b2Vec2*/,
	
	"b2internal var",{ m_vertexCount/*:int*/:0},
	 ...
	 */
	"static public function ComputeCentroid",function ComputeCentroid(vs/*:Array*//*b2Vec2*/, count/*:uint*/)/* : b2Vec2*/
	{
		//b2Settings.b2Assert(count >= 3);
		
		//b2Vec2 c; c.Set(0.0f, 0.0f);
		var c/*:b2Vec2*/ = new Box2D.Common.Math.b2Vec2();
		var area/*:Number*/ = 0.0;
	 ...
	 */
	"static b2internal function ComputeOBB",function ComputeOBB(obb/*:b2OBB*/, vs/*:Array*//*b2Vec2*/, count/*:int*/)/* : void*/
	{
		var i/*:int*/;
