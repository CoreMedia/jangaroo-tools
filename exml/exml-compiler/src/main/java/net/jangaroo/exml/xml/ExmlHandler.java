 ...
 */
public final class ExmlHandler extends CharacterRecordingHandler {

  public static final String EXML_NAMESPACE_URI = "http://net.jangaroo.com/extxml/0.1";

  private static final String NUMBER_PATTERN = "-?([0-9]+\\.[0-9]*)|(\\.[0-9]+)|([0-9]+)([eE][+-]?[0-9]+)?";
  ...
  private Stack<Json> objects = new Stack<Json>();
  private Stack<String> attributes = new Stack<String>();

  private boolean expectObjects = true;

  public ExmlHandler(ConfigClassRegistry registry) {
  ...
  public void startElement(String uri, String localName, String qName, Attributes atts) throws SAXException {
    if (EXML_NAMESPACE_URI.equals(uri)) {
      ...
      if (expectObjects) {
        ...
        //find description for namespace + localName:
        ConfigClass configClass = registry.getConfigClassByName(uri + "." + localName);
        if (configClass != null) {
          imports.add(configClass.getComponentName());
          jsonObject.setType(configClass.getName());
        } else {
  ...
  private JsonObject createJsonObject(Attributes atts) {
    ...
    for (int i = 0; i < atts.getLength(); i++) {
      ...
      } else if (attsValue.matches(NUMBER_PATTERN)) {
        try {
